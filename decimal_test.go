...
func init() {
	for _, s := range testTable {
		s.exact = strconv.FormatFloat(s.float, 'f', 2000, 64)
...
func TestNewFromFloatRandom(t *testing.T) {
	...
	for {
		...
		want, err := NewFromString(strconv.FormatFloat(in, 'f', 2000, 64))
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestNewFromFloatQuick(t *testing.T) {
	err := quick.Check(func(f float64) bool {
		want, werr := NewFromString(strconv.FormatFloat(f, 'f', 2000, 64))
		if werr != nil {
			return true
		}
		got := NewFromFloat(f)
...
func TestNewFromFloat32Random(t *testing.T) {
	...
	for {
		...
		want, err := NewFromString(strconv.FormatFloat(float64(in), 'f', 500, 32))
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestNewFromFloat32Quick(t *testing.T) {
	err := quick.Check(func(f float32) bool {
		want, werr := NewFromString(strconv.FormatFloat(float64(f), 'f', 500, 32))
		if werr != nil {
			return true
		}
...
func ExampleNewFromFloat32() {
	...
	// OUTPUT:
	//123.1231231689453125
	//0.12312312424182891845703125
	//-9999999827968
}
...
func ExampleNewFromFloat() {
	...
	// OUTPUT:
	//123.1231231231229941158744622953236103057861328125
	//0.12312312312312299888983346818349673412740230560302734375
	//-10000000000000
}

func TestNewFromFloatLoses(t *testing.T) {
	// This number is exactly representable as a float64:
	f64 := float64(rand.Int63())

	t.Logf("Number: %f", f64)

	fromInt := NewFromInt(int64(f64))
