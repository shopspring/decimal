...
func TestNewFromBigRat(t *testing.T) {
	...
	tests := map[Inp]string{
		{big.NewRat(0, 1), 16}:                                    "0",
		{big.NewRat(4, 5), 16}:                                    "0.8",
		{big.NewRat(10, 2), 16}:                                   "5",
		{big.NewRat(1023427554493, 43432632), 16}:                 "23563.5628642767953828", // rounded
		{big.NewRat(1, 434324545566634), 16}:                      "0.0000000000000023",
		{big.NewRat(1, 3), 16}:                                    "0.3333333333333333",
		{big.NewRat(2, 3), 2}:                                     "0.67",               // rounded
		{big.NewRat(2, 3), 16}:                                    "0.6666666666666667", // rounded
		{big.NewRat(10000, 3), 16}:                                "3333.3333333333333333",
		{mustParseRat("30702832066636633479"), 16}:                "30702832066636633479",
		{mustParseRat("487028320159896636679.1827512895753"), 16}: "487028320159896636679.1827512895753",
		{mustParseRat("127028320612589896636633479.173582751289575278357832"), -2}: "127028320612589896636633500",                  // rounded
		{mustParseRat("127028320612589896636633479.173582751289575278357832"), 16}: "127028320612589896636633479.1735827512895753", // rounded
		{mustParseRat("127028320612589896636633479.173582751289575278357832"), 32}: "127028320612589896636633479.173582751289575278357832",
	}
...
func TestDecimal_Pow(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"3.0", "-6.0", "0.001371742112482853"},
		{"13.757", "-5.0", "0.00000202946382098"},
		{"66.12", "-7.61313", "0.000000000000013853780421217382344176"},
		{"6696871.12", "-2.61313", "0.00000000000000000145579281162977937"},
		{"-3.0", "-6.0", "0.001371742112482853"},
		{"-13.757", "-5.0", "-0.00000202946382098"},
	} {
...
func TestDecimal_PowInt32(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"3.0", -6, "0.001371742112482853"},
		{"-13.757", -5, "-0.00000202946382098"},
	} {
...
func TestDecimal_PowBigInt(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"3.0", big.NewInt(-6), "0.001371742112482853"},
		{"-13.757", big.NewInt(-5), "-0.00000202946382098"},
	} {
		base, _ := NewFromString(testCase.Decimal)
		expected, _ := NewFromString(testCase.Expected)
